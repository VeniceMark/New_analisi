# File aggiornato: include clienti presenti solo nelle Effettive (union index) + filtro cliente sidebar
...
if sezione == "\U0001F4DD Budget Editor":
    ...
    if uploaded_budget:
        try:
            ...
            if "cliente" not in df.columns.str.lower():
                # prova a rinominare 'Cliente' -> 'cliente'
                rename_map = {c: "cliente" for c in df.columns if c.lower() == "cliente"}
    ...
    with st.form("aggiungi_cliente"):
        ...
        mesi = st.multiselect("Mesi da includere", options=list(range(1, 13)), default=list(range(1, 12+1)))
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            # parsing data (supporta sia gg-mm-aaaa che yyyy-mm-dd)
            ...
            # trova nome colonna cliente a prescindere dal case
            ...
            # === MODIFICA CHIAVE: usa l'unione degli indici per includere clienti solo-effettivo ===
            ...
            # Calcolo scostamenti percentuali (incluso "Extrabudget")
            ...
            # === Filtro cliente nella SIDEBAR (applica a tutte le viste) ===
            ...
            dashboard_f = dashboard.loc[dashboard.index.isin(idx)].sort_values(by="Scostamento %", ascending=True)

...
if sezione == "📝 Budget Editor":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            diff_percent_str = diff_percent.replace("Zero", "0%")
            diff_percent_num = budget.copy()

            for col in colonne_comuni:
                diff_percent_num[col] = pd.to_numeric(
                    diff_percent[col].replace("Extrabudget", np.nan).replace("Zero", 0).str.rstrip('%'),
                    errors='coerce'
                )

            df_to_show = diff_percent_num.copy()
            for col in colonne_comuni:
                df_to_show[col] = diff_percent_str[col]

            styled_diff = df_to_show.style.applymap(colori_scostamenti)
            st.dataframe(styled_diff, use_container_width=True)

            st.subheader("📋 Dati Dettagliati")
            df_view_formattato = df_view.copy()
            df_view_formattato.loc[:, ("Scostamento %", *colonne_comuni)] = df_view_formattato.loc[:, ("Scostamento %", *colonne_comuni)].replace("Zero", "0%")

            styled_view = df_view_formattato.style.applymap(colori_scostamenti, subset=pd.IndexSlice[("Scostamento %", *colonne_comuni)])

...
def create_summary_table(budget_data, pivot_1_15, pivot_totale):
    ...
    for cliente in sorted(all_clients):
        ...
        results.append({
            ...
            'Scostamento (num)': float(scostamento[:-1]) if isinstance(scostamento, str) and scostamento.endswith('%') else (-999 if scostamento == 'extrabudget' else (999 if scostamento == 'None' else None))
        })
        ...
        if budget_data and periods:
            ...
            with tab2:
                ...
                if not summary_df.empty:
                    ...
                    st.dataframe(style_dataframe(summary_df.drop(columns=['Scostamento (num)']), exclude_columns=['Budget', 'Effettivo']), use_container_width=True)

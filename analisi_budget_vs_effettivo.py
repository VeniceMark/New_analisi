# Analisi Budget vs Effettivo — v1.12-fix
# - Correzione SyntaxError (backslash nelle f-string)
# - round(2) interno per ore (effettivo/budget) e somme
# - Percentuali numeriche con 1 decimale; 'None' (nero su nero) per zero-zero; 'Extrabudget' (viola) per budget=0 & eff>0
...
st.set_page_config(page_title="Analisi Budget vs Effettivo (v1.12-fix)", layout="wide")
st.markdown("### 📊 Analisi Budget vs Effettivo — **v1.12-fix**")
...
# ------------------------------
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            # ---- DASHBOARD PER CLIENTE (robust dtypes)
            ...
            ore_eff = (eff_f[cols_fine].sum(axis=1) if len(cols_fine)>0 else eff_f.sum(axis=1)).astype(float).round(2)
            ore_bud = (budget_f[cols_fine].sum(axis=1) if len(cols_fine)>0 else budget_f.sum(axis=1)).astype(float).round(2)
            ...
            dashboard["Scostamento Valore (ore)"] = (dashboard["Ore a Budget"] - dashboard["Ore Effettive"]).astype(float).round(2)
            ...
            # percentuale robusta (evita dtype object)
            ...
            # sentinelle per casi particolari
            sc_perc.loc[(den_dash == 0) & (dashboard["Ore Effettive"] > 0)] = -9999
            # den==0 & eff==0 -> NaN (None)

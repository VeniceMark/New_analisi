    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            for col in colonne_comuni:
                ...
                perc_num = perc.copy()
                perc_num[mask_extrabudget] = -9999
                diff_percent_num[col] = perc_num
                perc_str = perc_num.astype(object)
                perc_str = perc_str.apply(lambda v: "Extrabudget" if v == -9999 else ("0%" if abs(v) < 1e-9 else f"{v:.1f}%"))

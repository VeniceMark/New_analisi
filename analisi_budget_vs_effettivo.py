...
if sezione == "📝 Budget Editor":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            colonne_budget_valide = [col for col in budget.columns if col.endswith("(1-fine)")]
            colonne_effettivo_valide = [col for col in eff.columns if col.endswith("(1-fine)")]
            budget_filtrato = budget[colonne_budget_valide]
            eff_filtrato = eff[colonne_effettivo_valide]
            dashboard = pd.DataFrame({
                "Ore Effettive": eff_filtrato.sum(axis=1),
                "Ore a Budget": budget_filtrato.sum(axis=1)
            })

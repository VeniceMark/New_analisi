# v3.0 - Analisi Budget vs Effettivo (categorie obbligatorie + filtri + viste complete)
# - Gate categorizzazione clienti solo-effettivo o senza categoria
# - Filtri Sidebar: Cliente, Categoria, Anno, Mese, colonne 1-15 / 1-fine
# - Heatmap scostamenti %, Dati Dettagliati, Dashboard per cliente, Riepilogo mensile
# - Colorazione: gradiente -50% -> +100%, Viola per Extrabudget, Nero per Zero
# - Safe division (nessuna divisione per zero)
# - Compatibilità st.rerun / st.experimental_rerun
...
st.set_page_config(page_title="Analisi Budget vs Effettivo (v3)", layout="wide")
st.markdown("### 📊 Analisi Budget vs Effettivo — **v3.0**")
...
if sezione == "📝 Budget Editor":
    ...
    if uploaded_budget:
        try:
            ...
            # Trova colonna cliente (case-insensitive)
            ...
            if not cat_col:
                ...
                # normalizza nome colonna per coerenza
                if cat_col != "categoria_cliente":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            def col_selected(c):
                ...
                y, m, half = p
                if y not in sel_years or m not in sel_months: return False
                if half == "1-15" and not include_115: return False
                if half == "1-fine" and not include_1fine: return False
                return True

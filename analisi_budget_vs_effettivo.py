...
if sezione == "📝 Budget Editor":
    ...
    periodo_scelto = st.sidebar.selectbox("🎯 Seleziona periodo da analizzare", ["Tutto"] + list(df_budget.columns if df_budget is not None else []))
    # Filtra colonne periodo valide
    colonne_periodi_valide = [col for col in df_budget.columns if re.match(r"^\\d{4}-\\d{2} \\(1-(15|fine)\\)$", col)]
    periodo_scelto = st.sidebar.selectbox("🗓️ Seleziona periodo", ["Tutto"] + sorted(colonne_periodi_valide))
    clienti_lista = sorted(df_budget.index.unique())
    cliente_scelto = st.sidebar.selectbox("👤 Filtra per cliente", ["Tutti"] + clienti_lista, index=0)

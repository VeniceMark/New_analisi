...
if uploaded_file:
    try:
        ...
        if not {'cliente', 'data', 'ore'}.issubset(df_eff.columns):
            ...
            scostamenti = []
            for idx in dashboard.index:
                effettivo = dashboard.at[idx, "Ore Effettive"]
                budgetato = dashboard.at[idx, "Ore a Budget"]
                if budgetato == 0 and effettivo > 0:
                    scostamenti.append("extrabudget")
                elif budgetato == 0 and effettivo == 0:
                    scostamenti.append(np.nan)
                else:
                    scostamenti.append(round((budgetato - effettivo) / budgetato * 100, 1))
            dashboard["Scostamento %"] = scostamenti

            dashboard[["Ore Effettive", "Ore a Budget", "Differenza (ore)"]] = dashboard[["Ore Effettive", "Ore a Budget", "Differenza (ore)"]].round(1)
            ...
            dashboard_sorted = dashboard.copy()
            try:
                dashboard_sorted["__sort__"] = dashboard_sorted["Scostamento %"].apply(lambda x: -999 if x == "extrabudget" else x)
                dashboard_sorted = dashboard_sorted.sort_values("__sort__").drop(columns="__sort__")
            except:
                pass

            st.dataframe(
                dashboard_sorted.style.format({
                    "Ore Effettive": "{:.1f}",
                    "Ore a Budget": "{:.1f}",
                    "Differenza (ore)": "{:.1f}",
                    "Scostamento %": lambda v: v if isinstance(v, str) else f"{v:.1f}%"
                }).applymap(color_dashboard),
                use_container_width=True
            ) else f"{v:.1f}%"}).applymap(color_dashboard),
                use_container_width=True
            )

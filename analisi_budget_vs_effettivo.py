...
if sezione == "\U0001F4DD Budget Editor":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            # Calcolo sicuro della % scostamento nel dashboard (no division by zero)
            num_dash = (dashboard["Ore a Budget"].astype(float) - dashboard["Ore Effettive"].astype(float)).astype(float)
...
/ dashboard["Ore a Budget"] * 100).round(1),

...
def colore_dashboard(riga):
    if riga["Ore a Budget"] == 0 and riga["Ore Effettive"] > 0:
...
if uploaded_file:
    try:
        ...
        styled = diff_numeric.style.background_gradient(
            ...
            lambda x: f"{x:.1f}%" if pd.notnull(x) else "").applymap(
            lambda val: "background-color: mediumpurple; color: white", subset=diff_display == "extrabudget"
        )
        ...
        styled_dashboard = dashboard[["Ore Effettive", "Ore a Budget", "Scostamento %"]].style             .apply(colore_dashboard, axis=1)             .background_gradient(subset=["Scostamento Valore"], cmap="RdYlGn", vmin=-50, vmax=100)

...
if uploaded_budget_file is not None and uploaded_effettivo_file is not None:
    try:
        ...
        df_effettivo_clean = df_effettivo_clean[(df_effettivo_clean['cliente'] != 'cliente') & (df_effettivo_clean['ore'] != 'ore')]

        # Gestione del formato data GG/MM/AA
        def parse_custom_date(date_str):
            try:
                # Assumendo che gli anni 25-30 siano 2025-2030
                day, month, year = map(int, date_str.split('/'))
                if year < 100:
                    if 25 <= year <= 39: # Modifica qui se l'intervallo cambia
                        year += 2000
                    else:
                        year += 1900 # Se necessario per anni < 25
                return pd.Timestamp(year=year, month=month, day=day)
            except:
                return pd.NaT

        df_effettivo_clean['data'] = df_effettivo_clean['data'].astype(str).apply(parse_custom_date)
        ...
        # Creazione Pivot per Effettivo 1-15 e 1-fine
        ...
        df_budget_clean.columns = df_budget_clean.columns.astype(str) # Assicura che i nomi siano stringhe
        df_budget_clean.rename(columns={df_budget_clean.columns[0]: 'cliente'}, inplace=True)

        # Identificazione colonne di budget ore
        ...
        clienti_budget = df_budget_ore['cliente'].unique()
        clienti_eff = pd.concat([pivot_eff_1_15['cliente'], pivot_eff_1_fine['cliente']]).unique()
        tutti_clienti = sorted(list(set(clienti_budget) | set(clienti_eff)))
        ...
        for cliente in tutti_clienti:
            ...
            for periodo in periodi:
                ...
                if "(1-15)" in periodo:
                    effettivo_val = eff_1_15_dict.get((cliente, periodo.split(' (')[0]), 0)
                elif "(1-fine)" in periodo:
                    effettivo_val = eff_1_fine_dict.get((cliente, periodo.split(' (')[0]), 0)
                else:
                    effettivo_val = 0 # Caso non previsto
                ...
                if budget_val == 0 and effettivo_val > 0:
                    ...
                    valore_cella = "None" # Fallback, anche se le regole dovrebbero coprire tutti i casi
        ...
        for cliente in tutti_clienti:
            ...
            # Somma totale Budget ed Effettivo
            tot_budget = sum(budget_cliente_dict.get(p, 0) for p in periodi)
            
            # Somma totale Effettivo (deve aggregare i valori dai pivot)
            tot_eff_1_15 = pivot_eff_1_15[pivot_eff_1_15['cliente'] == cliente]['ore_eff_1_15'].sum()
            tot_eff_1_fine = pivot_eff_1_fine[pivot_eff_1_fine['cliente'] == cliente]['ore_eff_1_fine'].sum()
            
            # Per la tabella B, sommiamo tutti gli effettivi 1-15 e 1-fine
            tot_effettivo = tot_eff_1_15 + tot_eff_1_fine # Oppure scegliere uno dei due se serve un totale distinto

            # Per Tabella B, calcoliamo un unico scostamento totale
            # Seguendo la logica del documento: Differenza (Budget - Effettivo), Scostamento % ((Budget - Effettivo) / Budget) * 100
            ...
            if tot_budget == 0:
                ...
                scostamento_tot_percentuale = np.nan # O un valore speciale se preferisci
            else:
                scostamento_tot_percentuale = (diff_budget_eff / tot_budget) * 100
                scostamento_tot_testo = scostamento_tot_percentuale # Per la visualizzazione colorata
            ...
            table_b_data.append({
                ...
                'Scostamento %': scostamento_tot_testo # Usa il testo o la percentuale per il colore
            })
        ...
        def highlight_cells_table_a(val):
            ...
            if isinstance(val, str):
                ...
                # Gradiente rosso-verde per percentuali
                # Limitiamo il range per il colore tra -100% e 100%
                perc = np.clip(val, -100, 100)
                # Normalizzazione tra 0 e 1
                norm = (perc + 100) / 200 
                # Interpolazione RGB: Rosso (255, 0, 0) a Verde (0, 255, 0)
                red = int(255 * (1 - norm))
                green = int(255 * norm)
                blue = 0
                style = f'background-color: rgb({red}, {green}, {blue}); color: black;'
                # Aggiungi il valore formattato
                # Nota: Streamlit non renderizza direttamente HTML in dataframes con st.dataframe
                # Quindi il valore numerico rimane, il colore è applicato tramite styler
            return style

        def highlight_cells_table_b(val, col_name):
             """Applica colori alle celle della Tabella B in base alla colonna 'Scostamento %'."""
             style = ''
        ...
        st.dataframe(
            df_table_a_filtered.style.applymap(highlight_cells_table_a),
            height=600,

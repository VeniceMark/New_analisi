...
# ------------------------------
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            # ---- Scostamenti % (stringhe con colori) - versione safe senza divisione per zero
            diff_percent = pd.DataFrame(index=eff.index, columns=colonne_comuni, dtype=object)
            for col in colonne_comuni:
                # calcolo percentuale in modo sicuro
                num = (budget[col] - eff[col]).astype(float)
                ...
                # inizialmente formatto come stringa con %
                perc_str = perc.astype(str) + "%"
                # sovrascrivo i casi speciali

# v3.3 - Unica colonna 'Scostamento %' (numerica) nelle tabelle: Dashboard per cliente + Riepilogo mensile
# - Ordinamento numerico corretto
# - Formattazione visuale: -9999 -> 'Extrabudget', 0 -> '0%', altrimenti 'x.y%'
# - Colorazione robusta: viola per -9999 o 'Extrabudget'; gradiente per i numeri
...
st.set_page_config(page_title="Analisi Budget vs Effettivo (v3.3)", layout="wide")
st.markdown("### 📊 Analisi Budget vs Effettivo — **v3.3**")
...
def colori_scostamenti(val):
    # Viola per Extrabudget (stringa) o per il sentinel numerico -9999
    ...
    # Nero per 'Zero' usato nella heatmap (non applicato alla dashboard riepilogativa)
    if isinstance(val, str) and val.strip().lower() == "zero":
        return 'background-color: black; color: white;'
    try:
        # Gestisce sia numeri sia stringhe con '%'
...
# ------------------------------
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            # ---- Scostamenti % (safe) + numeric backing
            ...
            for col in colonne_comuni:
                ...
                # build Series aligned to eff.index
                perc_s = pd.Series(perc, index=eff.index)

                mask_extrabudget = (den == 0) & (eff[col] > 0)
                mask_zero = (den == 0) & (eff[col] == 0)
                diff_mask_zero[col] = pd.Series(mask_zero, index=eff.index)

                # mark zero-zero as NaN, extrabudget as -9999
                perc_s.loc[mask_zero] = np.nan
                perc_s.loc[mask_extrabudget] = -9999
                diff_percent_num[col] = perc_s.astype(float)

                # pretty string (only for display, not for sorting)
                def _fmt(v):
                    # Use 'None' for NaN
                    try:
                        if pd.isna(v):
                    ...
                    if abs(fv) < 1e-9:
                        return "0%"
                    return f"{fv:.1f}%"
                diff_percent[col] = perc_s.map(_fmt)
# ------------------------------
            # FILTRI (sidebar)
            # ------------------------------
            ...
            # Anni & Mesi (dentro l'expander dell'anno)
            ...
            # ---- HEATMAP (ordinabile, numeric-backed)
            ...
            def _style_heatmap(_df):
                ...
                for c in _df.columns:
                    for r in _df.index:
                        ...
                        elif bool(zero_mask_f.loc[r, c]):
                            styles.loc[r, c] = 'background-color: black; color: white;'
                        else:
                            try:
                                norm = (float(val) + 50) / 150
                                norm = max(0.0, min(1.0, norm))
                                color = plt.cm.RdYlGn(norm)
            ...
            styled_diff = diff_num_f.style.apply(_style_heatmap, axis=None).format(fmt_percent_numeric)
            st.dataframe(styled_diff, use_container_width=True)

            # ---- DETTAGLIO COMPLETO (ordinabile, numeric-backed)
            st.subheader("📋 Dati Dettagliati (Effettivo / Budget / Scostamento %)")
            df_view = pd.concat([eff_f, budget_f, diff_percent_num.loc[idx, selected_cols]], keys=["Effettivo", "Budget", "Scostamento %"], axis=1)
            scostamento_cols = [col for col in df_view.columns if isinstance(col, tuple) and col[0] == "Scostamento %"]

            # Row-wise styling on the % subset so we can consult zero_mask_f per cell
            def _row_style_detail(row):
                styles = []
                rname = row.name
                for col in row.index:
                    # 'col' is a MultiIndex tuple: ("Scostamento %", "<mese half>")
                    mese = col[1]
                    val = row[col]
                    try:
                        fv = float(val)
                        ...
                        fv = None
                    if fv == -9999:
                        styles.append('background-color: violet; color: white;')
                    elif fv is not None and bool(zero_mask_f.loc[rname, mese]):
                        ...
                        try:
                            ...
                            norm = max(0.0, min(1.0, norm))
                            color = plt.cm.RdYlGn(norm)
                            styles.append(f'background-color: {matplotlib.colors.rgb2hex(color)}')
                        except Exception:
                            styles.append("")
                return styles

            fmt_dict = {("Scostamento %", c): fmt_percent_numeric for c in selected_cols}
            styled_view = df_view.style.apply(_row_style_detail, subset=pd.IndexSlice[:, scostamento_cols], axis=1).format(fmt_dict)
            ...
            def format_scostamento(val):
                # Non altera il valore numerico; solo formattazione in output
                if isinstance(val, (int, float)) and val == -9999:
                    return "Extrabudget"
                elif isinstance(val, (int, float)) and abs(val) < 1e-9:
                    return "0%"
                else:
                    return f"{val:.1f}%"

            styled_dashboard = dashboard.style.applymap(colori_scostamenti, subset=["Scostamento %"]).format({"Scostamento %": format_scostamento})
            ...
            styled_riep = riepilogo_mensile.style.applymap(colori_scostamenti, subset=["Scostamento %"]).format({"Scostamento %": format_scostamento})

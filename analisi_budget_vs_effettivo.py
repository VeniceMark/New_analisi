...
def evidenzia_viola_extrabudget(df_display: pd.DataFrame) -> pd.DataFrame:
    styles = pd.DataFrame("", index=df_display.index, columns=df_display.columns)
    for row in df_display.index:
        for col in df_display.columns:
            if df_display.at[row, col] == "extrabudget":
...
if uploaded_file:
    try:
        ...
        style_overlay = evidenzia_viola_extrabudget(diff_display)
        ...
        styled_dashboard = dashboard[["Ore Effettive", "Ore a Budget", "Scostamento %"]].style             .apply(evidenzia_dashboard_viola, axis=1)             .background_gradient(subset=["Scostamento Valore"], cmap="RdYlGn", vmin=-50, vmax=100)

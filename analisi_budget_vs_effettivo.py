...
if sezione == "\U0001F4DD Budget Editor":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            cliente_selezionato = st.sidebar.selectbox("Filtra per cliente", ["Tutti"] + clienti_unici)
            periodo_selezionato = st.sidebar.selectbox("Filtra per periodo", periodi_dropdown)

            if cliente_selezionato != "Tutti":
                eff = eff.loc[[cliente_selezionato]]
                budget = budget.loc[[cliente_selezionato]]

            if periodo_selezionato != "Tutto":

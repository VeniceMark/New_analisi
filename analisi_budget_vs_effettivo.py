    ...
    try:
        ...
        if not {'cliente', 'data', 'ore'}.issubset(df_eff.columns):
            ...
            if comuni.empty:
                ...
                st.subheader("📊 Grafico a barre: Budget vs Effettivo per tutti i clienti")
                mese_selezionato = st.selectbox("Seleziona un mese", comuni.unique())
                if mese_selezionato in comuni:

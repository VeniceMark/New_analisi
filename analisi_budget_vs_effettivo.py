...
# Resto del codice escluso per limiti di spazio.
# Sarà completato nel prossimo step.
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            st.success("✅ File 'Effettivo' elaborato correttamente.")

        except Exception as e:
            st.error(f"Errore durante l'elaborazione del file Effettivo: {e}")
            ...
            if colonne_comuni.empty:
                st.warning("⚠️ Nessuna colonna comune trovata tra Effettivo e Budget. Controlla che i mesi coincidano (es. '2025-07 (1-fine)').")
                st.stop()

            if eff.empty or budget.empty:
            ...
            tabella_unificata[("Totale", "% Totale")] = np.select(
                [
                    (totale_budget == 0) & (totale_eff == 0),
                    (totale_budget > 0) & (totale_budget == totale_eff),
                    (totale_budget > 0) & (totale_budget != totale_eff)
                ],
                ["none", "extrabudget", "extrabudget"],
                default=np.round((totale_budget - totale_eff) / totale_budget * 100, 1).astype(str) + "%"
            )
            ...
            def colori_scostamenti_totale(val):
                if val == "extrabudget":
                    return 'background-color: violet; color: white;'
                elif val == "none":
                    return 'background-color: black; color: white;'
            ...
            styled = styled.applymap(colori_scostamenti_totale, subset=pd.IndexSlice[:, pd.IndexSlice["Totale", "% Totale"]])

...
def ui_toggle(label: str, value: bool, key: str):
    # Compatibilità: usa st.toggle se presente, altrimenti st.checkbox
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            if selezione_cliente != "Tutti i clienti":
                ...
                idx = [c for c in eff.index if cat_map.get(c, "") in categorie_scelte]
            ...
            dashboard = pd.DataFrame({
                ...
                "Categoria": [df_budget.loc[c, "categoria_cliente"] if c in df_budget.index else "" for c in eff_f.index]
            }, index=eff_f.index)

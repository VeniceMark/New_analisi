...
if sezione == "📝 Budget Editor":
    ...
    periodo_scelto = st.sidebar.selectbox("🎯 Seleziona periodo da analizzare", ["Tutto"] + sorted(colonne_periodi_valide))
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            if 'cliente' in df_budget.columns:
                ...
                st.error("❌ Il foglio 'Budget' deve contenere una colonna chiamata 'cliente'.")
                st.stop()
            ...
            zero_zero_matrix = (budget == 0) & (eff == 0)
            ...
            def colori_scostamenti(val, is_zero_zero):
                if isinstance(val, str):
                    if val == "Extrabudget":
                        return 'background-color: violet; color: white;'
                    elif val == "0%" and is_zero_zero:
            ...
            def color_matrix_wrapper(val, row, col):
                return colori_scostamenti(val, zero_zero_matrix.loc[row, col])
            styled_diff = diff_percent.copy()
            styled_diff = styled_diff.style.apply(lambda df: [[color_matrix_wrapper(df.loc[i, j], i, j) for j in df.columns] for i in df.index], axis=None)
            ...
            stacked_zero_flag = zero_zero_matrix.stack()
            st.dataframe(df_view.style.applymap(colori_scostamenti, subset=pd.IndexSlice[:, "Scostamento %"]), use_container_width=True)
            def color_dettagliati(val, idx):
                flag = stacked_zero_flag.get(idx, False)
                return colori_scostamenti(val, flag)
            styled_view = df_view.copy()
            styled_view = styled_view.style.apply(lambda df: [color_dettagliati(v, i) for i, v in df.items()] if df.name == "Scostamento %" else ["" for _ in df], axis=1)
            st.dataframe(styled_view, use_container_width=True)

            # ➕ Nuova sezione: dashboard riepilogativa per cliente
            st.subheader("📊 Dashboard riepilogativa per cliente")
            dashboard = pd.DataFrame({
            zero_zero_case = (dashboard["Ore a Budget"] == 0) & (dashboard["Ore Effettive"] == 0)
            ...
            def color_wrapper(val, idx):
                return colori_scostamenti(val, zero_zero_case.loc[idx])
            styled_dashboard = styled_dashboard.apply(lambda col: [color_wrapper(v, i) for i, v in zip(dashboard.index, col)], subset=["Scostamento %"])

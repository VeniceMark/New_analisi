...
if sezione == "📝 Budget Editor":
    ...
    if st.session_state["budget_df"] is not None:
        ...
        if uploaded_budget:
            ...
            df_budget.columns = df_budget.columns.map(str).str.strip().str.lower()
            if 'cliente' not in df_budget.columns:
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            df_eff.columns = df_eff.columns.map(str).str.strip().str.lower()
            if 'cliente' not in df_eff.columns:
                st.error("❌ Il foglio 'Effettivo' deve contenere una colonna chiamata 'cliente'.")
                st.stop()

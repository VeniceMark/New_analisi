...
if sezione == "📝 Budget Editor":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            diff_valori = pd.DataFrame(index=budget.index, columns=budget.columns, dtype=float)
            diff_etichetta = pd.DataFrame(index=budget.index, columns=budget.columns, dtype=object)

            for col in colonne_comuni:
                diff_valori[col] = np.where(
                    (budget[col] == 0) & (eff[col] > 0), -9999,
                    np.where((budget[col] == 0) & (eff[col] == 0), 0,
                             ((budget[col] - eff[col]) / budget[col] * 100).round(1))
                )
                diff_etichetta[col] = np.where(
                    diff_valori[col] == -9999, "Extrabudget",
                    np.where(diff_valori[col] == 0, "Zero", diff_valori[col].astype(str) + "%")
                )

            styled_diff = diff_etichetta.style.applymap(colori_scostamenti)
            styled_diff = styled_diff.format(lambda v: "0%" if v == "Zero" else v)
            st.dataframe(styled_diff, use_container_width=True)

            st.subheader("📋 Dati Dettagliati")
            df_view = pd.concat([eff, budget, diff_etichetta], keys=["Effettivo", "Budget", "Scostamento %"], axis=1)

...
if sezione == "📝 Budget Editor":
    ...
    if uploaded_eff and df_budget is not None:
        try:
            ...
            # Costruzione versione numerica e stringa per ordinamento + visualizzazione
            diff_percent_num = budget.copy()

            for col in colonne_comuni:
                diff_percent_num[col] = np.where(
            ...
            diff_percent_str = diff_percent_num.applymap(format_percent)

            styled_diff = diff_percent_str.style.applymap(colori_scostamenti)
            st.dataframe(styled_diff, use_container_width=True)

            st.subheader("📋 Dati Dettagliati")
            df_view_num = pd.concat([eff, budget, diff_percent_num], keys=["Effettivo", "Budget", "Scostamento %"], axis=1)
            df_view_str = pd.concat([eff, budget, diff_percent_str], keys=["Effettivo", "Budget", "Scostamento %"], axis=1)

            scostamento_cols = [col for col in df_view_str.columns if isinstance(col, tuple) and col[0] == "Scostamento %"]
            styled_view = df_view_str.style.applymap(colori_scostamenti, subset=pd.IndexSlice[:, scostamento_cols])
            ...
            dashboard["Scostamento % (num)"] = np.where(
            ...
            dashboard["Scostamento % (vis)"] = dashboard["Scostamento % (num)"].apply(format_scostamento)
            dashboard = dashboard[~((dashboard["Ore Effettive"] == 0) & (dashboard["Ore a Budget"] == 0))]
            dashboard = dashboard.sort_values(by="Scostamento % (num)", ascending=True)

            styled_dashboard = dashboard.style.applymap(colori_scostamenti, subset=["Scostamento % (vis)"])
